                                ...
                                <div class="span12">
									...
									<a href="teacher_exam.php" class="btn btn-info"><i class="icon-arrow-left"></i> Back</a>
										...
										<div class="control-group">
											...
											<select name="exam_id">
											<option></option>
												<?php  $query = mysqli_query($conn,"select * from exam where teacher_id = '$session_id'")or die(mysqli_error());
												while ($row = mysqli_fetch_array($query)){ $id = $row['exam_id']; ?>
												<option value="<?php echo $id; ?>"><?php echo $row['exam_title']; ?></option>
												<?php } ?>
											</select>
											</div>
										</div>
				...
				<tbody>
					...
					<?php } ?>
										...
										if (isset($_POST['save'])){
											...
											for($i=0; $i < $N; $i++)
											{
												mysqli_query($conn,"insert into class_exam (teacher_class_id,exam_time,exam_id) values('$id[$i]','$time','$exam_id')")or die(mysqli_error());
												mysqli_query($conn,"insert into notification (teacher_class_id,notification,date_of_notification,link) value('$id[$i]','$name_notification',NOW(),'student_exam_list.php')")or die(mysqli_error());
											} ?>
											<script>
