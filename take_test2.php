...
<?php $get_id = $_GET['id']; ?>
<?php $class_exam_id = $_GET['class_exam_id']; ?>
<?php $exam_id = $_GET['exam_id']; ?>
<?php $exam_time = $_GET['exam_time']; ?>

<?php $query1 = mysqli_query($conn,"select * from student_class_exam where student_id = '$session_id' and class_exam_id = '$class_exam_id' ")or die(mysqli_error());
...
if ($count > 0){
 ...
 mysqli_query($conn,"insert into student_class_exam (class_exam_id,student_id,student_exam_time) values('$class_exam_id','$session_id','$exam_time')");
}
 ?>
...
$sqlp = mysqli_query($conn,"SELECT * FROM class_exam WHERE class_exam_id = '$class_exam_id'")or die(mysqli_error());
...
<form action="take_test2.php<?php echo '?id='.$get_id; ?>&<?php echo 'class_exam_id='.$class_exam_id; ?>&<?php echo 'test=done' ?>&<?php echo 'exam_id='.$exam_id; ?>&<?php echo 'exam_time='.$exam_time; ?>" name="testform" method="POST" id="test-form">
<?php
										$sqla = mysqli_query($conn,"select * FROM class_exam 
										...
										/* $rowa   = $row['exam_id']; */
/* $sqla = mysqli_query($conn,"SELECT * FROM class_exam WHERE course_code = '".$row['course_code']."'"); */

?>
										<h3>Test Title: <b><?php echo $rowa['exam_title'];?></b></h3>
										<p><b>Description:<?php echo $rowa['exam_description'];?></b></p>
										<p></p>
...
<?php
	$sqlw = mysqli_query($conn,"SELECT * FROM exam_question where exam_id = '$exam_id'  ORDER BY RAND()");
...
?>
	<input name="q-<?php echo $roww['exam_question_id'];?>" value="True" type="radio"> True&nbsp;|&nbsp;<input name="q-<?php echo $roww['exam_question_id'];?>" value="False" type="radio"> False
<?php
} else if($roww['question_type_id']=='1') {
	$sqly = mysqli_query($conn,"SELECT * FROM answer WHERE exam_question_id = '".$roww['exam_question_id']."'");
	while($rowy = mysqli_fetch_array($sqly)){
	if($rowy['choices'] == 'A') {
	?>
	A.)<input name="q-<?php echo $roww['exam_question_id'];?>" value="A" type="radio"> <?php echo $rowy['answer_text'];?><br /><br />
	<?php } else if ($rowy['choices'] == 'B') {?>                                 
	B.) <input name="q-<?php echo $roww['exam_question_id'];?>" value="B" type="radio"> <?php echo $rowy['answer_text'];?><br /><br />
	<?php } else if ($rowy['choices'] == 'C') {?>                                 
	C.) <input name="q-<?php echo $roww['exam_question_id'];?>" value="C" type="radio"> <?php echo $rowy['answer_text'];?><br /><br />
	<?php } else if ($rowy['choices'] == 'D') {?>                                 
	D.) <input name="q-<?php echo $roww['exam_question_id'];?>" value="D" type="radio"> <?php echo $rowy['answer_text'];?><br /><br />
	<?php
		}
	}
}
...
<input type="hidden" name="x-<?php echo $x;?>" value="<?php echo $roww['exam_question_id'];?>">
</td>
</tr>
<?php
	}
...
<?php
	...
	for($x=1;$x<=$x1;$x++){
		...
		$sql = mysqli_query($conn,"SELECT * FROM exam_question WHERE exam_question_id = ".$x2."");
	...
	<a href="student_exam_list.php<?php echo '?id='.$get_id; ?>"><i class="icon-arrow-left"></i> Back</a>
	<center>
	<h3><br>Your score is <b><?php echo $score; ?></b> out of <b><?php echo ($x-1); ?></b><br/></h3>
	</center>
	<?php
	/* echo "Your Percentage Grade is : <b>".$per."%</b>"; */
	mysqli_query($conn,"UPDATE student_class_exam SET `student_exam_time` = 3600, `grade` = '".$score." out of ".($x-1)."' WHERE student_id = '$session_id' and class_exam_id = '$class_exam_id'")or die(mysqli_error());
?>
<script>
	  window.location = 'student_exam_list.php<?php echo '?id='.$get_id; ?>'; 
</script>
<?php
	} /* else { */
?>
