...
<?php $get_id = $_GET['id']; ?>
<?php $exam_question_id = $_GET['exam_question_id']; ?>
<body>
                     ...
                     <div class="row-fluid">
                        ...
                        <div id="block_bg" class="block">
                            <div class="navbar navbar-inner block-header">
								...
								<a href="exam_question.php<?php echo '?id='.$get_id; ?>" class="btn btn-success"><i class="icon-arrow-left"></i> Back</a>
								</div>
                            </div>
                            <div class="block-content collapse in">
                                <div class="span12">
								<?php
										$query = mysqli_query($conn,"select * FROM exam_question
										LEFT JOIN question_type on exam_question.question_type_id = question_type.question_type_id
										where exam_id = '$get_id' and exam_question_id = '$exam_question_id'  order by date_added DESC ")or die(mysqli_error());
							    ...
							    <form class="form-horizontal" method="post">
										...
										<div id="opt11">
<?php
	$sqlz = mysqli_query($conn,"SELECT * FROM answer2 WHERE exam_question_id = ".$_GET['id']."");
...
<div id="opt12">
	...
	<input name="correctt" <?php if($row['answer'] == 'False'){ echo 'checked'; }?> value="f" type="radio">False<br /><br />
		...
		if ($type  == '2'){
				mysqli_query($conn,"insert into exam_question (exam_id,question_text,date_added,answer,question_type_id) 
		...
		$query = mysqli_query($conn,"select * from exam_question order by exam_question_id DESC LIMIT 1")or die(mysqli_error());
		$row = mysqli_fetch_array($query);
		$exam_question_id = $row['exam_question_id'];
		
		mysqli_query($conn,"insert into answer (exam_question_id,answer_text,choices) values('$exam_question_id','$ans1','A')")or die(mysqli_error());
		mysqli_query($conn,"insert into answer (exam_question_id,answer_text,choices) values('$exam_question_id','$ans2','B')")or die(mysqli_error());
		mysqli_query($conn,"insert into answer (exam_question_id,answer_text,choices) values('$exam_question_id','$ans3','C')")or die(mysqli_error());
		mysqli_query($conn,"insert into answer (exam_question_id,answer_text,choices) values('$exam_question_id','$ans4','D')")or die(mysqli_error());
